        ...
        if match:
            ...
            with open(file_path, 'r') as f:
                ...
                if dimension is not None and topk is not None:
                    ...
                    if recall is not None: # Store recall
        ...
        for member in tar.getmembers():
            if member.name.endswith('.log'):
                ...
                if match:
                    ...
                    recall = parse_log_file(file_content_bytes) # Get only recall
...
def main():
    ...
    dataset = base_name.split('-')[1] if '-' in base_name else 'unknown'
    # Update defaultdict to only include recalls list
    ...
    # Save results to CSV
    csv_filename = f'recall-{dataset}.csv'
    with open(csv_filename, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        # Write header - remove time columns
        writer.writerow(['workload_type', 'dimension', 'topk', 'M', 'ef', 'avg_recall(%)'])

        # Write data
        for (workload_type, dim, topk, m, ef), data in sorted_results: # Renamed 'times' to 'data'
            # Skip if missing recall data
            if not data['recalls']:
                print(f"Skipping entry due to missing recall data: {(workload_type, dim, topk, m, ef)}")
                continue
            avg_recall = sum(data['recalls']) / len(data['recalls']) # Calculate average recall
            # Write recall to CSV, remove time data
            writer.writerow([workload_type, dim, topk, m, ef, f"{avg_recall:.3f}"])
